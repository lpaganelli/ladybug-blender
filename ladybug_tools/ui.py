...
class LB_PT_honeybee(LBPanel, bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        row.operator('ladybug.hb_constructions', icon='MATERIAL')
        ...
        if p.en_hvac == 'FREE_RUNNING':
            ...
            box.prop(p, 'en_conditioned', placeholder='all occupied rooms')
        box.prop(p, 'en_open_doors', placeholder='PM01, PM04')
        row = box.row(align=True)
